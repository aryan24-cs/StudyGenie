...
import { FaBrain, FaFire, FaBullseye, FaClock, FaCalendar, FaBolt, FaChartBar, FaTrophy, FaUserCircle, FaTrash, FaEye, FaSignOutAlt } from 'react-icons/fa';

function Dashboard() {
    const navigate = useNavigate();
    const [history, setHistory] = useState([
        { _id: '1', fileName: 'Math_Notes.pdf', filePath: '/uploads/math_notes.pdf', uploadDate: '2025-08-20T10:00:00Z' },
        { _id: '2', fileName: 'Physics_Quiz.pdf', filePath: '/uploads/physics_quiz.pdf', uploadDate: '2025-08-19T14:30:00Z' },
        { _id: '3', fileName: 'Chemistry_Chapter1.pdf', filePath: '/uploads/chem_ch1.pdf', uploadDate: '2025-08-18T09:15:00Z' },
    ]);
    ...
    const recentActivity = [
        { date: 'Today', activity: 'Completed 25 flashcards in Physics', score: '+120 XP' },
        { date: 'Yesterday', activity: 'Perfect score on Chemistry quiz', score: '+200 XP' },
        { date: '2 days ago', activity: 'Studied for 2.5 hours', score: '+150 XP' },
        { date: '3 days ago', activity: 'Unlocked new Math chapter', score: '+100 XP' },
    ];

    const achievements = [
        { title: 'Study Streak', description: '12 days in a row', icon: FaFire, color: '#ff4d4f' },
        { title: 'Quiz Master', description: '10 perfect scores', icon: FaTrophy, color: '#f4b400' },
        { title: 'Speed Learner', description: '100 cards in 1 hour', icon: FaBolt, color: '#666' },
        { title: 'Knowledge Seeker', description: '5 subjects mastered', icon: FaBrain, color: '#2196f3' },
    ];
    ...
    return (
        <div className="dashboard-main">
            ...
            <header className="dashboard-header">
                ...
                <button className="logout-btn pulse" onClick={handleLogout}>
                        ...
                        <div className="heatmap-card animate-on-scroll">
                            ...
                            <div className="heatmap-legend">
                                ...
                                <div className="legend-squares">
                                    {[0, 1, 2, 3, 4].map((level) => (
                                        <div key={level} className={`legend-square activity-${level}`} />
                                    ))}
                                </div>
                                <span>More</span>
                            </div>
                    ...
                    <section className="goal-section">
                        ...
                        <div className="goal-card animate-on-scroll">
                            <div className="goal-header">
                                <span>{userStats.hoursStudied} / {userStats.weeklyGoal} hours</span>
                                <span className="goal-percentage">
                                    {Math.round((userStats.hoursStudied / userStats.weeklyGoal) * 100)}% complete
                                </span>
                            </div>
                            <div className="progress-bar">
                                <div className="progress-fill" style={{ width: `${(userStats.hoursStudied / userStats.weeklyGoal) * 100}%` }}></div>
                            </div>
                            <p className="goal-remaining">
                                {Math.max(0, userStats.weeklyGoal - userStats.hoursStudied).toFixed(1)} hours remaining
                            </p>
                        </div>
                    </section>
                                    ...
                                    <tbody>
                                        {history.map((item) => (
                                            <tr key={item._id}>
                                                ...
                                                <td>
                                                    <button className="view-btn" onClick={() => handleViewItem(item)} aria-label={`View ${item.fileName}`}>
                                                        <FaEye /> View
                                                    </button>
                                                    <button
                ...
                <div className="right-column">
                    ...
                    <section className="level-section">
                        ...
                        <div className="level-card animate-on-scroll">
                            ...
                            <p>{userStats.xp} XP</p>
                            <div className="progress-bar">
                                <div className="progress-fill" style={{ width: `${(userStats.xp % 1000) / 10}%` }}></div>
                            </div>
                            <p className="xp-remaining">{1000 - (userStats.xp % 1000)} XP to next level</p>
                        </div>
                    </section>
